...
import { logError, logMessage, logUserAction } from '../utils/logger';
		...
		return (
			<div className='flex-1 flex flex-col items-center justify-center p-8'>
				<div className='max-w-2xl text-center'>
					...
					<h1 className='text-3xl font-bold mb-4'>Olá! Eu sou o Mangaba Assistente</h1>
			...
			{dragOver && (
				<div className='absolute inset-0 bg-gradient-to-br from-green-500/20 via-yellow-500/15 to-red-500/20 dark:bg-black/80 border-2 border-dashed border-green-500 dark:border-white rounded-lg flex items-center justify-center z-50'>
					<div className='text-center'>
						...
						<p className='text-sm text-muted-foreground'>Imagens, PDFs, textos (máx. 10MB)</p>
					</div>
				</div>
			)}

			{/* Header */}
			<div className='flex items-center justify-between p-4 border-b border-border flex-shrink-0 bg-gradient-to-r dark:from-inherit dark:via-inherit from-green-50/50 via-yellow-50/40 to-red-50/50 '>
				<div className='flex items-center space-x-3'>
					...
					<div>
						<h1 className='text-lg font-semibold text-foreground'>{selectedAgent?.name || 'Assistente'}</h1>
				...
				<AnimatePresence>
					{currentChat.messages.map((msg) => (
						<motion.div
							...
							exit={{ opacity: 0, y: -20 }}
							className={cn('flex gap-2 sm:gap-4', msg.role === 'user' ? 'justify-end' : 'justify-start')}
						>
							{msg.role === 'assistant' && (
						...
						<div className='flex items-center gap-2'>
							...
							>
								{isRecording ? (
									...
									<Mic size={16} className='sm:w-[18px] sm:h-[18px] text-muted-foreground' />
								)}
							</button>
